...
internal class CommitsHistoryReportGenerator : IReportGenerator
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var scriptCommand = CommandLineHelper.IsWindowsPlatform()
            ...
            var scriptErrors = SplitIntoLines(await standardError.WaitAsync(cancellationToken));
            var scriptOutput = SplitIntoLines(await standardOutput.WaitAsync(cancellationToken));
